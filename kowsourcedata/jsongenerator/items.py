    ...
    for filename in listdir(data_location.csv.rules):
        if "items" in filename and filename.endswith('.csv'):
            with open(path.join(data_location.csv.rules, filename), 'r') as file:
                ...
                line = file.readline()
                while line:
                    name, key, pts, modifier, limitation, version, status, description, armylist_description = csvutils.get_elements(line);
                    ...
                    armylist_description = "" # Armylist description of this item
